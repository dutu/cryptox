...
function Bitfinex (options) {
    ...
    var _mytrades  = function (symbol, options, callback) {         // same function as native API wrapper, but it returns result according to JSON schema
        bitfinexPrivate.mytrades(symbol, options, function (err, xTransactions) {
            var trade;
            var result = {
                timestamp: util.timestampNow(),
                error: err && err.message || "",
    ...
    self.getTransactions = function (options, callback) {
        ...
        if (options.hasOwnProperty("skip"))
            skip = options.skip;
        async.series({
                getMovements: function(cb){          // Initialize database
	                xOptions = {};
                    if (gotTheMovements)
	                ...
	                var currency1, currency1, currencies = [];
	                if (pOptions.hasOwnProperty("symbol")) {
		                currency1 = pOptions.symbol(0,3);
		                currency2 = pOptions.symbol(3,3);
