...
function Btce (options) {
    ...
    self.getTicker = function (options, callback) {
        ...
        btcePublic.ticker(btcePair, function (err, btceTicker) {
            ...
            if (err) {
                ticker = {
                    timestamp: util.unixTimestampNow(),
                    error: err.message,
                    data: {}
                }
            } else {
    ...
    self.getOpenOrders = function (options, callback) {
        ...
        btcePrivate.activeOrders(pair, function (err, result) {
            openOrders.timestamp = util.unixTimestampNow();      // we update the tiomestamp due to the callback
            if (!err) {
                ...
                for (var record in activeOrders) {
                    var newOrder = {
                        ...
                        timestamp: util.adjTimestamp(activeOrders[record].timestamp_created)      // 1396619879
                    };
