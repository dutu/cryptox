...
function Bitstamp (options) {
    ...
    self.getTransactions = function (options, callback) {
	    ...
	    if (options.hasOwnProperty("before") || options.hasOwnProperty("after") || options.hasOwnProperty("type")) {
		    ...
		    xOptions.limit = self.properties.dictionary.user_transactions.limit.maximum;
		    xOptions.offset = 0;
		    manualFilter = true;
	    }
	    async.doWhilst(
		    function (cb) {
			    user_transactions(xOptions, function (err, result) {
				    if (err)
				        return cb(err);
				    fetchMore = !(xOptions.limit < self.properties.dictionary.user_transactions.limit.maximum);     // if we fetched less than maximum we don't need to fetchMore for sure
