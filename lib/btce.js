...
function Btce (options) {
    ...
    self.getOpenOrders = function (options, callback) {
        ...
        btcePrivate.activeOrders(pair, function (err, result) {
            ...
            if (!err) {
                // var jf = require("jsonfile"); jf.writeFileSync(__dirname + "/bitstamp-getTransactions_MockApiResponse-user_transactions.json", result);     // only used to create MockApiResponse file for the test unit
