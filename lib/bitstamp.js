...
function Bitstamp (options) {
    ...
    self.getTicker = function (options, callback) {
        ...
        let ticker, data;
        let pair = (options.pair || '').replace(/xbt/i, 'btc').toLocaleLowerCase();
        bitstampPublic.ticker(pair, function(xErr, bitstampTicker) {
            ...
            // var jf = require("jsonfile"); jf.writeFileSync(__dirname + "/bitstamp-getFee_MockApiResponse.json", bitstampTicker);     // only used to create MockApiResponse file for the test unit
            ticker.timestamp = util.timestamp(bitstampTicker.timestamp);
            data = {
                pair: (options.pair || '').replace(/btc/i, 'xbt').toUpperCase(),
    ...
    self.getFee = function (options, callback) {
        ...
        bitstampPrivate.balance('', function (xErr, bitstampBalance) {
            ...
            //let jf = require("jsonfile"); jf.writeFileSync(__dirname + "/bitstamp-getFee_MockApiResponse.json", bitstampBalance);     // only used to create MockApiResponse file for the test unit
            ...
            if (pair) {
                ...
                for (let key in bitstampBalance) {
                    if (bitstampBalance.hasOwnProperty(key)) {
                        ...
                        if (s[1] === 'fee') {
                            let data = {
                                pair: (s[0]).replace(/btc/i, 'xbt').toUpperCase(),
