...
var util = {
    ...
    extendTransactions: function (target, newTransactions) {
        // Merge the contents of two transaction results into the first one
        var i, p, push, pDatetime, iDatetime;
        p = 0;
        for (i = 0; i < newTransactions.data.length; i++) {
            push = false;
            while (!push && p < target.data.length) {
                pDatetime = moment (target.data[p].datetime);
                iDatetime = moment (newTransactions.data[i].datetime);
                if (iDatetime.isAfter(pDatetime))
                    push = true;
                if (!push && iDatetime.isSame(pDatetime) && newTransactions.data[i].tx_id.localeCompare(target.data[p].tx_id) > 0)
                    // if same datetime, push if i.tx_id sorts after p.tx_id
                    push = true;
                if (!push)
                    p++;
            }
