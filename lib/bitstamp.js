...
function Bitstamp (options) {
    ...
    self.getTransactions = function (options, callback) {
        var limit, skip, skippedCount, beforeMoment, afterMoment, txMoment, fetchMore;
	    ...
	    var xOptions = {
		    offset: !options.hasOwnProperty("before") && skip || 0,
		    limit: limit,
		    sort: options.sort
	    };
	    if (options.hasOwnProperty("before") || options.hasOwnProperty("after"))
		    xOptions.limit = self.properties.dictionary.user_transactions.limit.maximum;        // if before or after parameters are present we always fetch the maximum number of transactions

        user_transactions(xOptions, function (err, result) {
	        fetchMore = result.data.length < limit;     // if fetched below the xOptions.limit, we can't fetch more (have fetched all)
	        // drop records before, after and skip
	        var p = 0;  // points to the current record to check;
	        while (p < result.data.length) {
		        var drop = false;
		        txMoment = moment(result.data[p].datetime);
		        drop = beforeMoment && txMoment.isAfter(beforeMoment);    // need to drop transaction after this date
		        if (!drop && skippedCount < skip) {
			        drop = true;
			        skippedCount++
		        }
		        drop = drop || afterMoment && txMoment.isBefore(afterMoment);   // need to drop transaction before this date
		        if (drop)
			        result.data.splice(p, 1);       // drop the record
		        else
		            p++;                           // or move pointer to next one
	        }
	        fetchMore = fetchMore && result.data.length >= limit;     // if fetched below the xOptions.limit, we can't fetch more (have fetched all)
	        if (!fetchMore && result.data.length > limit) {   //apply limit
		        result.data.splice(limit, result.data.length - limit);
	        }
            callback(err, result);
        });
    };
